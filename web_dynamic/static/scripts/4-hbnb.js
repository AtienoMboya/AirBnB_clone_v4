$('document').ready(function () {
  ...
  for (let i = 0; i < amenityCheckboxArray.length; i++) {
    amenityCheckboxArray[i].addEventListener('change', () => {
      if ($(amenityCheckboxArray[i]).is(':checked')) {
        ...
        idList.push($(amenityCheckboxArray[i]).attr('data-id'));
      } else {
        delete amenityChecked[$(amenityCheckboxArray[i]).attr('data-id')];
        const index = idList.indexOf($(amenityCheckboxArray[i]).attr('data-id'));
        if (index > -1) { // only splice idList when item is found
          idList.splice(index, 1); // 2nd parameter means remove one item only
        }
      }
  ...
  $('BUTTON').click(() => {
    const placesData = { amenities: idList };
